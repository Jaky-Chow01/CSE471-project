...
<body class="min-h-screen pb-24 relative">
    ...
    <nav class="px-4 sm:px-6 py-4 flex items-center justify-between max-w-7xl mx-auto">
        ...
        <a href="{{ route('home') }}" class="text-sm font-bold text-gray-600 hover:text-red-600 transition">← Back</a>
    </nav>
    ...
    <div class="max-w-2xl mx-auto px-4 mt-6">
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[3rem] shadow-xl border border-white overflow-hidden">
            ...
            <div class="flex items-center gap-4 px-6 py-4 {{ !$loop->last ? 'border-b border-gray-100' : '' }} {{ $index < 3 ? 'bg-gradient-to-r from-red-50/50 to-transparent' : '' }} hover:bg-red-50/30 transition">
                ...
                <div class="flex-1 min-w-0">
                    <p class="font-extrabold text-gray-800 truncate">{{ $donor->name }}</p>
                    <p class="text-xs text-gray-400 truncate">{{ $donor->email }}</p>
                </div>
                <div class="flex-shrink-0 text-right">
                    <p class="font-extrabold text-red-600 text-lg">{{ $donor->total_bags ?? 0 }}</p>
