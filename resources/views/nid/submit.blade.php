...
<body class="min-h-screen pb-20 relative">
    ...
    @php $isDonor = auth()->user()->isDonor(); @endphp
    ...
    <nav class="px-4 sm:px-6 py-4 flex items-center justify-between max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            <a href="{{ $isDonor ? route('donor.dashboard') : route('requester.dashboard') }}" class="hidden sm:inline text-sm font-bold text-gray-500 hover:text-red-600 transition">My Panel</a>
    ...
    <div class="max-w-xl mx-auto px-4 mt-4 space-y-5">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ $isDonor ? route('donor.dashboard') : route('requester.dashboard') }}"
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-emerald-200 shadow-xl p-8 text-center">
            ...
            <p class="text-xs text-gray-400 mt-1">Verified on {{ $existing->verified_at?->format('d M Y') ?? 'N/A' }}</p>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-amber-200 shadow-xl p-8 text-center">
            ...
            <p class="text-xs text-gray-400 mt-1">Submitted on {{ $existing->created_at->format('d M Y') }}</p>
            <div class="mt-4 p-3 bg-gray-50 rounded-2xl text-left space-y-1">
                <p class="text-xs font-bold text-gray-600">Name: {{ $existing->full_name }}</p>
                <p class="text-xs font-bold text-gray-600">NID: {{ $existing->nid_number }}</p>
            </div>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-white shadow-xl p-8">
            ...
            <form method="POST"
                  action="{{ $isDonor ? route('donor.nid.store') : route('requester.nid.store') }}"
                ...
                <div>
                    ...
                    <input type="text" name="full_name" value="{{ old('full_name', $existing->full_name ?? '') }}" required
                ...
                <div>
                    ...
                    <input type="text" name="nid_number" value="{{ old('nid_number', $existing->nid_number ?? '') }}" required
