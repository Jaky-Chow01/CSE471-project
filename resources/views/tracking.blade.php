...
<body class="min-h-screen pb-20 relative">
    ...
    <nav class="px-4 sm:px-6 py-4 flex items-center justify-between max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            @if($role === 'admin')
                <a href="{{ route('admin.dashboard') }}" class="hidden sm:inline text-sm font-bold text-gray-500 hover:text-red-600 transition">Admin Panel</a>
            ...
            @else
                <a href="{{ route('login') }}" class="text-sm font-bold text-gray-500 hover:text-red-600 bg-white/70 px-4 py-2 rounded-2xl border border-white transition">
    ...
    <div class="max-w-3xl mx-auto px-4 space-y-5 mt-2">
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-xl border border-white p-8 text-center">
            ...
            <h2 class="text-4xl font-extrabold text-red-600">{{ $stages[$stage]['label'] }}</h2>
            <p class="text-sm text-gray-500 mt-2">Step {{ $stage + 1 }} of 5 &nbsp;·&nbsp; Live synchronization active</p>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-xl border border-white p-6 sm:p-8">
            ...
            <div class="relative flex items-start justify-between">
                ...
                <div class="absolute left-0 top-5 h-1 bg-emerald-500 rounded-full transition-all duration-700" style="width: {{ $stage * 25 }}%"></div>
                @for($i = 0; $i < 5; $i++)
                ...
                <div class="relative z-10 flex flex-col items-center w-1/5">
                    <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center border-2 transition-all duration-300
                        {{ $stage >= $i ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-200' : 'bg-white border-gray-200 text-gray-300' }}">
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-xl border border-blue-100 p-6">
            ...
            <div class="space-y-3">
                @for($i = 0; $i < 5; $i++)
                @php $info = $stages[$i]; @endphp
                <div class="flex items-start gap-3 p-4 rounded-2xl {{ $stage === $i ? 'bg-blue-50 border border-blue-200' : ($stage < $i ? 'opacity-40' : 'bg-gray-50') }}">
                    <div class="mt-0.5 w-6 h-6 rounded-lg flex-shrink-0 flex items-center justify-center text-[10px] font-black
                        ...
                        @if($stage > $i)<i class="fa-solid fa-check"></i>@elseif($stage === $i)<i class="fa-solid fa-circle-dot"></i>@else{{ $i + 1 }}@endif
                    </div>
                    <div><p class="text-xs font-black {{ $stage === $i ? 'text-blue-700' : 'text-gray-600' }}">{{ $info['label'] }}</p><p class="text-xs text-gray-500 leading-relaxed mt-0.5">{{ $info['donor_msg'] }}</p></div>
                </div>
                @endfor
            </div>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-xl border border-red-100 p-6">
            ...
            <div class="space-y-3">
                @for($i = 0; $i < 5; $i++)
                @php $info = $stages[$i]; @endphp
                <div class="flex items-start gap-3 p-4 rounded-2xl {{ $stage === $i ? 'bg-red-50 border border-red-200' : ($stage < $i ? 'opacity-40' : 'bg-gray-50') }}">
                    <div class="mt-0.5 w-6 h-6 rounded-lg flex-shrink-0 flex items-center justify-center text-[10px] font-black
                        {{ $stage >= $i ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-400' }}">
                        @if($stage > $i)<i class="fa-solid fa-check"></i>@elseif($stage === $i)<i class="fa-solid fa-circle-dot"></i>@else{{ $i + 1 }}@endif
                    </div>
                    <div><p class="text-xs font-black {{ $stage === $i ? 'text-red-700' : 'text-gray-600' }}">{{ $info['label'] }}</p><p class="text-xs text-gray-500 leading-relaxed mt-0.5">{{ $info['requester_msg'] }}</p></div>
                </div>
                @endfor
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-blue-100 p-6">
                ...
                <div class="space-y-2">
                    @for($i = 0; $i < 5; $i++) @php $info = $stages[$i]; @endphp
                    <div class="flex items-start gap-2 p-3 rounded-xl {{ $stage === $i ? 'bg-blue-50 border border-blue-200' : ($stage < $i ? 'opacity-40' : '') }}">
                        <div class="mt-0.5 w-5 h-5 rounded-md flex-shrink-0 flex items-center justify-center text-[9px] font-black {{ $stage >= $i ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-400' }}">
                            @if($stage > $i)<i class="fa-solid fa-check"></i>@elseif($stage === $i)<i class="fa-solid fa-circle-dot"></i>@else{{ $i + 1 }}@endif
                        </div>
                        <div><p class="text-[10px] font-black {{ $stage === $i ? 'text-blue-700' : 'text-gray-600' }}">{{ $info['label'] }}</p><p class="text-[10px] text-gray-500">{{ $info['donor_msg'] }}</p></div>
                    </div>
                    @endfor
                </div>
            </div>
            <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-red-100 p-6">
                ...
                <div class="space-y-2">
                    @for($i = 0; $i < 5; $i++) @php $info = $stages[$i]; @endphp
                    <div class="flex items-start gap-2 p-3 rounded-xl {{ $stage === $i ? 'bg-red-50 border border-red-200' : ($stage < $i ? 'opacity-40' : '') }}">
                        <div class="mt-0.5 w-5 h-5 rounded-md flex-shrink-0 flex items-center justify-center text-[9px] font-black {{ $stage >= $i ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-400' }}">
                            @if($stage > $i)<i class="fa-solid fa-check"></i>@elseif($stage === $i)<i class="fa-solid fa-circle-dot"></i>@else{{ $i + 1 }}@endif
                        </div>
                        <div><p class="text-[10px] font-black {{ $stage === $i ? 'text-red-700' : 'text-gray-600' }}">{{ $info['label'] }}</p><p class="text-[10px] text-gray-500">{{ $info['requester_msg'] }}</p></div>
                    </div>
                    @endfor
                </div>
            </div>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-white shadow-xl p-6">
            ...
            <div class="space-y-3">
                <div class="flex items-center gap-3 p-3 bg-blue-50 border border-blue-200 rounded-2xl">
                    ...
                    <a href="{{ $donor_link }}" target="_blank" class="text-xs text-blue-600 underline truncate flex-1">{{ $donor_link }}</a>
                    <button onclick="navigator.clipboard.writeText('{{ $donor_link }}')" class="flex-shrink-0 text-[10px] px-3 py-1.5 bg-blue-100 hover:bg-blue-200 text-blue-700 rounded-xl font-bold transition">Copy</button>
                </div>
                <div class="flex items-center gap-3 p-3 bg-red-50 border border-red-200 rounded-2xl">
                    ...
                    <a href="{{ $requester_link }}" target="_blank" class="text-xs text-red-600 underline truncate flex-1">{{ $requester_link }}</a>
                    <button onclick="navigator.clipboard.writeText('{{ $requester_link }}')" class="flex-shrink-0 text-[10px] px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 rounded-xl font-bold transition">Copy</button>
                </div>
            </div>
        </div>
        @else
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto">
                ...
                <div class="flex items-start gap-3 p-3 rounded-2xl {{ in_array($notif->status, ['sent','delivered']) ? 'bg-emerald-50 border border-emerald-200' : 'bg-red-50 border border-red-200' }}">
                    <div class="mt-0.5 flex-shrink-0">
                        @if($notif->channel === 'email')<i class="fa-solid fa-envelope text-blue-500 text-sm"></i>
                        @else<i class="fa-solid fa-bell text-amber-500 text-sm"></i>@endif
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span class="text-[10px] text-gray-400 ml-auto">Stage {{ $notif->stage }} · {{ \Carbon\Carbon::parse($notif->created_at)->format('H:i:s') }}</span>
                        </div>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-dashed border-gray-300 p-6">
            ...
            <div class="flex flex-wrap justify-center gap-3">
                <a href="{{ route('update.stage', 0) }}" class="px-5 py-2.5 bg-white border border-gray-300 rounded-2xl text-xs font-bold hover:bg-gray-50 transition"><i class="fa-solid fa-rotate-left mr-1 text-gray-400"></i>Reset</a>
                <a href="{{ route('update.stage', 1) }}" class="px-5 py-2.5 bg-blue-50 border border-blue-300 rounded-2xl text-xs font-bold text-blue-700 hover:bg-blue-100 transition"><i class="fa-solid fa-check mr-1"></i>Accept</a>
                <a href="{{ route('update.stage', 2) }}" class="px-5 py-2.5 bg-purple-50 border border-purple-300 rounded-2xl text-xs font-bold text-purple-700 hover:bg-purple-100 transition"><i class="fa-solid fa-hospital-user mr-1"></i>Arrived</a>
                <a href="{{ route('update.stage', 3) }}" class="px-5 py-2.5 bg-amber-50 border border-amber-300 rounded-2xl text-xs font-bold text-amber-700 hover:bg-amber-100 transition"><i class="fa-solid fa-vial-circle-check mr-1"></i>Matched</a>
                <a href="{{ route('update.stage', 4) }}" class="px-5 py-2.5 bg-red-600 text-white rounded-2xl text-xs font-black shadow-lg shadow-red-200 hover:bg-red-700 transition"><i class="fa-solid fa-heart mr-1"></i>Complete</a>
            </div>
        </div>
        @endif
        </div>{{-- end panel-tracking --}}
