    ...
    <div class="max-w-5xl mx-auto px-4 mt-4 space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-white/80 backdrop-blur-md rounded-[2rem] border border-white shadow-xl p-5 text-center">
                ...
                <p class="text-2xl font-extrabold text-gray-800">{{ $totalDonors }}</p>
            ...
            <div class="bg-white/80 backdrop-blur-md rounded-[2rem] border border-white shadow-xl p-5 text-center">
                ...
                <p class="text-2xl font-extrabold text-gray-800">{{ $totalRequesters }}</p>
                <p class="text-[10px] font-black uppercase tracking-widest text-gray-400 mt-1">Requesters</p>
            </div>
            <div class="bg-white/80 backdrop-blur-md rounded-[2rem] border {{ $pendingNid > 0 ? 'border-amber-200 bg-amber-50/80' : 'border-white bg-white/80' }} backdrop-blur-md shadow-xl p-5 text-center">
                <div class="w-10 h-10 rounded-2xl {{ $pendingNid > 0 ? 'bg-amber-100' : 'bg-gray-100' }} flex items-center justify-center mx-auto mb-2">
                    <i class="fa-solid fa-id-card {{ $pendingNid > 0 ? 'text-amber-600' : 'text-gray-500' }}"></i>
                </div>
                <p class="text-2xl font-extrabold {{ $pendingNid > 0 ? 'text-amber-700' : 'text-gray-800' }}">{{ $pendingNid }}</p>
            ...
            <div class="bg-white/80 backdrop-blur-md rounded-[2rem] border border-white shadow-xl p-5 text-center">
                ...
                <p class="text-2xl font-extrabold text-gray-800">{{ $totalDonors + $totalRequesters }}</p>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <a href="{{ route('admin.nid') }}"
               class="bg-white/80 backdrop-blur-md rounded-[2rem] border {{ $pendingNid > 0 ? 'border-amber-200' : 'border-white' }} shadow-xl p-5 flex flex-col items-center gap-3 hover:shadow-2xl hover:-translate-y-0.5 transition group relative">
                ...
                @if($pendingNid > 0)
                <span class="absolute top-3 right-3 bg-red-500 text-white text-[9px] font-black px-2 py-0.5 rounded-full">{{ $pendingNid }}</span>
                @endif
            </a>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-white shadow-xl p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('admin.users') }}" class="text-xs font-bold text-red-600 hover:text-red-700 transition">View all →</a>
            </div>
            <div class="space-y-2">
                @foreach($recentUsers as $user)
                <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl">
                    <div class="w-8 h-8 rounded-xl {{ $user->isDonor() ? 'bg-blue-100' : 'bg-red-100' }} flex items-center justify-center flex-shrink-0">
                        <i class="fa-solid {{ $user->isDonor() ? 'fa-user-nurse text-blue-600' : 'fa-person-half-dress text-red-600' }} text-xs"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-black text-gray-800 truncate">{{ $user->name }}</p>
                        <p class="text-xs text-gray-400 truncate">{{ $user->email }}</p>
                    </div>
                    <span class="text-[10px] font-black uppercase px-2 py-1 rounded-full {{ $user->isDonor() ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700' }}">{{ $user->role }}</span>
                </div>
                @endforeach
            </div>
        </div>
