    ...
    <div class="max-w-5xl mx-auto px-4 mt-4 space-y-5">
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-white shadow-xl overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                ...
                @php $nid = $user->nidVerification; @endphp
                <div class="flex items-center gap-3 px-4 sm:px-6 py-4 hover:bg-gray-50 transition">
                    <div class="w-10 h-10 rounded-2xl {{ $user->isDonor() ? 'bg-blue-100' : 'bg-red-100' }} flex items-center justify-center flex-shrink-0">
                        <i class="fa-solid {{ $user->isDonor() ? 'fa-user-nurse text-blue-600' : 'fa-person-half-dress text-red-600' }} text-sm"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-black text-gray-800 truncate">{{ $user->name }}</p>
                        <p class="text-xs text-gray-400 truncate">{{ $user->email }}</p>
                    </div>
                    <span class="hidden sm:inline text-[10px] font-black uppercase px-2.5 py-1 rounded-full {{ $user->isDonor() ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700' }}">
                        {{ $user->role }}
                    </span>
                    <span class="text-[10px] font-black uppercase px-2.5 py-1 rounded-full
                        @if(!$nid) bg-gray-100 text-gray-500
                        @elseif($nid->status === 'verified') bg-emerald-100 text-emerald-700
                        @elseif($nid->status === 'pending') bg-amber-100 text-amber-700
                        @else bg-red-100 text-red-700 @endif">
                        @if(!$nid) No NID @else {{ $nid->status }} @endif
                    </span>
                    <span class="hidden sm:inline text-[10px] text-gray-400 flex-shrink-0">{{ $user->created_at->format('d M Y') }}</span>
                </div>
                @endforeach
            </div>
