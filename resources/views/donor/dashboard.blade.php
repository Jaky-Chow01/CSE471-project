...
<body class="min-h-screen pb-20 relative">
    ...
    <nav class="px-4 sm:px-6 py-4 flex items-center justify-between max-w-4xl mx-auto">
        ...
        <div class="flex items-center gap-2 sm:gap-3">
            <span class="hidden sm:flex text-xs text-gray-400 font-mono bg-white/70 px-3 py-1.5 rounded-2xl border border-white items-center max-w-[160px] truncate">
                <i class="fa-solid fa-user-nurse mr-1 text-blue-500 flex-shrink-0"></i><span class="truncate">{{ auth()->user()->name }}</span>
            </span>
    ...
    <div class="max-w-3xl mx-auto px-4 mt-4 space-y-5">
        ...
        <div>
            <h1 class="text-2xl font-extrabold text-gray-800">Welcome, {{ auth()->user()->name }}</h1>
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-white shadow-xl p-6">
            ...
            <div class="flex items-center gap-3 p-3 bg-emerald-50 border border-emerald-200 rounded-2xl">
                ...
                <div>
                    ...
                    <p class="text-xs text-emerald-600">Verified on {{ $nid->verified_at?->format('d M Y') ?? 'N/A' }}</p>
                </div>
            </div>
            @elseif($nid->status === 'pending')
            <div class="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-2xl">
                ...
                <div>
                    ...
                    <p class="text-xs text-amber-600">Submitted on {{ $nid->created_at->format('d M Y') }}. Admin will review shortly.</p>
                </div>
            </div>
            @else
        ...
        <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-white shadow-xl p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="text-[10px] font-black uppercase px-2.5 py-1 rounded-full bg-red-100 text-red-700">
                    {{ $openRequests->count() }} waiting
                </span>
                @endif
            </div>

            @if($nid?->status !== 'verified')
            ...
            <div class="space-y-3">
                ...
                <div class="border border-gray-200 rounded-2xl p-4 bg-white hover:shadow-md transition">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-2xl bg-red-100 flex items-center justify-center flex-shrink-0">
                                <span class="text-xs font-black text-red-600">{{ $req->bloodRequest?->bloodgroup ?? '?' }}</span>
                            </div>
                            <div>
                                <p class="text-sm font-black text-gray-800">{{ $req->bloodRequest?->bloodgroup ?? 'Unknown' }} Blood Needed</p>
                                <p class="text-xs text-gray-500">
                                    <i class="fa-solid fa-location-dot mr-1"></i>{{ $req->bloodRequest?->location ?? 'N/A' }}
                                    &nbsp;·&nbsp;
                                    <i class="fa-solid fa-droplet mr-1"></i>{{ $req->bloodRequest?->noofbags ?? '?' }} bag(s)
                                </p>
                                @if($req->bloodRequest?->urgent === 'Urgent')
                                <span class="inline-block mt-1 text-[9px] font-black uppercase px-2 py-0.5 rounded-full bg-red-600 text-white">Urgent</span>
                                @endif
                            </div>
                        </div>
                        <form method="POST" action="{{ route('donor.request.accept', $req->id) }}" class="flex-shrink-0">
                            @csrf
                            <button type="submit"
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-100 flex flex-wrap items-center gap-2 sm:gap-4 text-[10px] text-gray-400">
                        <span><i class="fa-solid fa-calendar mr-1"></i>{{ $req->bloodRequest?->datetime ? \Carbon\Carbon::parse($req->bloodRequest->datetime)->format('d M Y, H:i') : 'N/A' }}</span>
                        <span><i class="fa-solid fa-person mr-1"></i>{{ ucfirst($req->bloodRequest?->patienttype ?? 'N/A') }}, {{ $req->bloodRequest?->patientage ?? '?' }} yrs</span>
                        <span class="sm:ml-auto">Request #{{ $req->id }}</span>
                    </div>
                </div>
                @endforeach
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center gap-3 p-3 bg-blue-50 border border-blue-200 rounded-2xl">
                    <div class="w-9 h-9 rounded-xl bg-blue-100 flex items-center justify-center flex-shrink-0">
                        <span class="text-[10px] font-black text-blue-700">{{ $req->bloodRequest?->bloodgroup ?? '?' }}</span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-xs font-black text-blue-700">Donation #{{ $req->id }}</p>
                        <p class="text-[10px] text-gray-500">
                            Stage {{ $req->stage }}/4 — {{ $stageLabels[$req->stage] ?? 'Unknown' }}
                            · {{ $req->created_at->format('d M Y') }}
                        </p>
                    </div>
                    <a href="{{ route('track.donor', $req->donor_token) }}" target="_blank"
