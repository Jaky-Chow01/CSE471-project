...
<body class="min-h-screen pb-24 relative">
    ...
    async function toggleAvail(id, btn) {
        const r = await fetch('/api/dashboard/donor/toggle', { method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'}, body:JSON.stringify({donor_id:id}) });
    ...
    async function registerDonor() {
        ...
        const r = await fetch('/api/dashboard/donor/register',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'},body:JSON.stringify({name,blood_group:blood,location,latitude:lat||23.8103,longitude:lng||90.4125,phone,email})});
        const d = await r.json();
    ...
    async function submitNewRequest() {
        ...
        const r = await fetch('/api/dashboard/requests/add',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'{{ csrf_token() }}'},body:JSON.stringify({bloodgroup:blood,noofbags:bags,location:hosp,contactno:contact,urgent:urgent?'Urgent':''})});
        const d = await r.json();
