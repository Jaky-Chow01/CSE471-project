...
<body class="min-h-screen pb-24 relative">
    ...
    <div class="max-w-6xl mx-auto px-4 mt-4">
        ...
        <div class="text-center mb-8">
            <h1 class="text-3xl font-black text-gray-800 mb-2">Blood Banks Near You</h1>
            <p class="text-gray-600 font-medium">Find emergency blood banks and contact them instantly</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="space-y-4">
                ...
                @forelse($bloodbanks as $bank)
                    <div onclick="showOnMap('{{ $bank->name }}', '{{ $bank->location }}')" class="bg-white/90 backdrop-blur-sm p-6 rounded-[2rem] shadow-sm border border-white hover:shadow-lg transition cursor-pointer">
                        <div class="flex justify-between items-start mb-4">
                            <div>
                                <h3 class="text-lg font-bold text-gray-900 mb-1">{{ $bank->name }}</h3>
                                <p class="text-gray-600 font-medium text-sm">{{ $bank->location }}</p>
                            </div>
                            <div class="flex gap-2">
                                <button onclick="event.stopPropagation(); showOnMap('{{ $bank->name }}', '{{ $bank->location }}')"
                                        class="bg-red-100 text-red-600 p-2 rounded-xl hover:bg-red-200 transition">
                                    <svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
                                    </svg>
                                </button>
                                <a href="tel:{{ $bank->contactno }}"
                        ...
                        <div class="flex items-center justify-between">
                            <div class="text-sm text-gray-500">
                                <span class="font-semibold">Contact:</span> {{ $bank->contactno }}
                            </div>
                            <button onclick="getDirections('{{ $bank->location }}')"
