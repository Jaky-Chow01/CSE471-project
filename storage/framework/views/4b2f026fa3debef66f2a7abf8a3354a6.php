...
<body class="min-h-screen pb-20 relative">
    ...
    <div class="max-w-5xl mx-auto px-4 mt-4 space-y-5">
        ...
        <div class="space-y-4">
            <?php $__currentLoopData = $pending; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nid): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="bg-white/80 backdrop-blur-md rounded-[2.5rem] border border-amber-200 shadow-xl p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-2xl <?php echo e($nid->user && $nid->user->isDonor() ? 'bg-blue-100' : 'bg-red-100'); ?> flex items-center justify-center flex-shrink-0">
                                <i class="fa-solid <?php echo e($nid->user && $nid->user->isDonor() ? 'fa-user-nurse text-blue-600' : 'fa-person-half-dress text-red-600'); ?> text-sm"></i>
                            </div>
                            <div>
                                <p class="text-sm font-black text-gray-800"><?php echo e($nid->user->name ?? 'Unknown'); ?></p>
                                <p class="text-xs text-gray-400"><?php echo e($nid->user->email ?? '—'); ?></p>
                            </div>
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-2xl">
                                ...
                                <span class="text-sm font-black text-gray-800"><?php echo e($nid->full_name); ?></span>
                            </div>
                            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-2xl">
                                <span class="text-xs text-gray-500 font-bold">NID Number</span>
                                <span class="font-mono text-sm font-black text-gray-800"><?php echo e($nid->nid_number); ?></span>
                            </div>
                            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-2xl">
                                <span class="text-xs text-gray-500 font-bold">Submitted</span>
                                <span class="text-xs font-bold text-gray-600"><?php echo e($nid->created_at->format('d M Y, H:i')); ?></span>
                            </div>
                        </div>

                        <div class="flex gap-3">
                            <form method="POST" action="<?php echo e(route('admin.nid.verify', $nid)); ?>" class="flex-1">
                                <?php echo csrf_field(); ?>
                                <button type="submit"
                            ...
                            <form method="POST" action="<?php echo e(route('admin.nid.reject', $nid)); ?>" class="flex-1">
                                <?php echo csrf_field(); ?>
                                <button type="submit"
                    ...
                    <div>
                        ...
                        <?php if($nid->image_path): ?>
                        <img src="<?php echo e(Storage::url($nid->image_path)); ?>"
                             alt="NID for <?php echo e($nid->full_name); ?>"
...
<?php /**PATH C:\xampp\htdocs\bloodConnect\resources\views/admin/nid-queue.blade.php ENDPATH**/ ?>