...
<body class="min-h-screen pb-24 relative">
    ...
    <nav class="px-4 sm:px-6 py-4 flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div class="hidden md:flex items-center gap-1">
            <a href="<?php echo e(route('find.donors')); ?>" class="px-4 py-2 text-sm font-bold text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-2xl transition">Find Donors</a>
            <a href="<?php echo e(route('leaderboard')); ?>" class="px-4 py-2 text-sm font-bold text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-2xl transition">Leaderboard</a>
            <?php if(auth()->guard()->check()): ?>
                <?php if(auth()->user()->isAdmin()): ?>
                    <a href="<?php echo e(route('admin.dashboard')); ?>" class="ml-2 px-4 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-2xl transition shadow-md shadow-red-200">Admin Panel</a>
                <?php elseif(auth()->user()->isDonor()): ?>
                    <a href="<?php echo e(route('donor.dashboard')); ?>" class="ml-2 px-4 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-2xl transition shadow-md shadow-red-200">My Panel</a>
                <?php else: ?>
                    <a href="<?php echo e(route('requester.dashboard')); ?>" class="ml-2 px-4 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-2xl transition shadow-md shadow-red-200">My Panel</a>
                <?php endif; ?>
            <?php else: ?>
                <a href="<?php echo e(route('login')); ?>" class="px-4 py-2 text-sm font-bold text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-2xl transition">Login</a>
                <a href="<?php echo e(route('register')); ?>" class="ml-2 px-4 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-2xl transition shadow-md shadow-red-200">Register</a>
            <?php endif; ?>
        </div>
        <!-- Mobile hamburger -->
        <button id="mobile-menu-btn" class="md:hidden p-2 rounded-xl hover:bg-red-50 transition" onclick="toggleMobileMenu()">
    ...
    <div id="mobile-menu" class="hidden fixed inset-0 z-50 md:hidden" onclick="if(event.target===this)toggleMobileMenu()">
        ...
        <div class="absolute top-0 right-0 w-72 h-full bg-white shadow-2xl flex flex-col">
            ...
            <div class="flex flex-col gap-1 p-4 flex-1">
                <a href="<?php echo e(route('find.donors')); ?>" class="px-4 py-3 text-sm font-bold text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-2xl transition">Find Donors</a>
                <a href="<?php echo e(route('leaderboard')); ?>" class="px-4 py-3 text-sm font-bold text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-2xl transition">Leaderboard</a>
                <div class="h-px bg-gray-100 my-2"></div>
                <?php if(auth()->guard()->check()): ?>
                    <?php if(auth()->user()->isAdmin()): ?>
                        <a href="<?php echo e(route('admin.dashboard')); ?>" class="px-4 py-3 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-2xl transition text-center">Admin Panel</a>
                    <?php elseif(auth()->user()->isDonor()): ?>
                        <a href="<?php echo e(route('donor.dashboard')); ?>" class="px-4 py-3 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-2xl transition text-center">My Panel</a>
                    <?php else: ?>
                        <a href="<?php echo e(route('requester.dashboard')); ?>" class="px-4 py-3 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-2xl transition text-center">My Panel</a>
                    <?php endif; ?>
                <?php else: ?>
                    <a href="<?php echo e(route('login')); ?>" class="px-4 py-3 text-sm font-bold text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-2xl transition">Login</a>
                    <a href="<?php echo e(route('register')); ?>" class="px-4 py-3 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-2xl transition text-center">Register</a>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-5xl mx-auto px-3 sm:px-4 grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
        <a href="<?php echo e(route('blood.types')); ?>" class="bg-white/60 backdrop-blur-md p-4 rounded-3xl border border-white flex items-center gap-3 hover:bg-white/90 transition shadow-sm">
            <div class="bg-red-100 p-3 rounded-2xl text-red-600 flex-shrink-0">
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C7 2 3 6 3 11s9 11 9 11 9-6 9-11-4-9-9-9zm0 15c-1.7 0-3-1.3-3-3s1.3-3 3-3 3 1.3 3 3-1.3 3-3 3z"/></svg>
            </div>
            <div>
                <p class="text-[10px] font-black text-gray-400 uppercase">Learn</p>
                <p class="text-sm font-bold text-gray-800">Blood Types</p>
            </div>
        </a>
        <a href="<?php echo e(route('blood.banks')); ?>" class="bg-white/60 backdrop-blur-md p-4 rounded-3xl border border-white flex items-center gap-3 hover:bg-white/90 transition shadow-sm">
            <div class="bg-red-100 p-3 rounded-2xl text-red-600 flex-shrink-0">
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L4.5 20.29l.71.71L12 18l6.79 3 .71-.71z"/></svg>
            </div>
            <div>
                <p class="text-[10px] font-black text-gray-400 uppercase">Emergency</p>
                <p class="text-sm font-bold text-gray-800">Blood Banks</p>
            </div>
        </a>
        <a href="<?php echo e(route('diagnostic.centers')); ?>" class="bg-white/60 backdrop-blur-md p-4 rounded-3xl border border-white flex items-center gap-3 hover:bg-white/90 transition shadow-sm">
            <div class="bg-red-100 p-3 rounded-2xl text-red-600 flex-shrink-0">
            ...
            <div>
                <p class="text-[10px] font-black text-gray-400 uppercase">Health Care</p>
                <p class="text-sm font-bold text-gray-800">Diagnostic Centers</p>
            </div>
        </a>
        <a href="<?php echo e(route('find.donors')); ?>" class="bg-white/60 backdrop-blur-md p-4 rounded-3xl border border-white flex items-center gap-3 hover:bg-white/90 transition shadow-sm">
            <div class="bg-blue-100 p-3 rounded-2xl text-blue-600 flex-shrink-0">
            ...
            <div>
                <p class="text-[10px] font-black text-gray-400 uppercase">Rameeza</p>
                <p class="text-sm font-bold text-gray-800">Find Donors</p>
            </div>
        </a>
        <?php if(auth()->guard()->check()): ?>
        <a href="<?php echo e(auth()->user()->isDonor() ? route('donor.dashboard') : (auth()->user()->isRequester() ? route('requester.dashboard') : route('admin.dashboard'))); ?>"
           class="bg-white/60 backdrop-blur-md p-4 rounded-3xl border border-white flex items-center gap-3 hover:bg-white/90 transition shadow-sm">
            <div class="bg-emerald-100 p-3 rounded-2xl text-emerald-600 flex-shrink-0">
            ...
            <div>
                <p class="text-[10px] font-black text-gray-400 uppercase">My Account</p>
                <p class="text-sm font-bold text-gray-800">My Panel</p>
            </div>
        </a>
        <?php else: ?>
        <a href="<?php echo e(route('login')); ?>" class="bg-white/60 backdrop-blur-md p-4 rounded-3xl border border-white flex items-center gap-3 hover:bg-white/90 transition shadow-sm">
            <div class="bg-emerald-100 p-3 rounded-2xl text-emerald-600 flex-shrink-0">
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>
            </div>
            <div>
                <p class="text-[10px] font-black text-gray-400 uppercase">Rameeza</p>
                <p class="text-sm font-bold text-gray-800">Track Donation</p>
            </div>
        </a>
        <?php endif; ?>
    </div>
    ...
    <div class="flex flex-col items-center justify-center mt-12 px-4 text-center">
        <div class="bg-white/70 backdrop-blur-md rounded-[2.5rem] shadow-sm py-6 sm:py-8 px-6 sm:px-10 max-w-2xl border border-white">
            <h1 class="text-lg sm:text-xl md:text-2xl font-bold text-gray-800 leading-tight">
    ...
    <div class="max-w-2xl mx-auto mt-16 px-4">
        ...
        <div class="space-y-4">
            <?php $__empty_1 = true; $__currentLoopData = $announcements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $request): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div onclick="openRequestModal(<?php echo e($request->id); ?>, '<?php echo e($request->bloodgroup); ?>', '<?php echo e($request->location); ?>', '<?php echo e($request->patienttype); ?>', '<?php echo e($request->patientage); ?>', '<?php echo e($request->contactno); ?>', '<?php echo e($request->urgent); ?>', '<?php echo e($request->created_at->format('Y-m-d H:i:s')); ?>')" class="bg-white/90 backdrop-blur-sm p-6 rounded-[2rem] shadow-sm border-l-8 <?php echo e($request->urgent == 'Urgent' ? 'border-red-600' : 'border-gray-300'); ?> cursor-pointer hover:shadow-lg transition-all duration-300 hover:scale-[1.02]">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                        <div class="space-y-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="text-lg sm:text-xl font-extrabold text-gray-900"><?php echo e($request->bloodgroup); ?> Needed</span>
                                <?php if($request->urgent == 'Urgent'): ?>
                                    <span class="bg-red-100 text-red-600 text-[10px] font-black px-2 py-0.5 rounded-full uppercase animate-pulse">Urgent</span>
                                <?php endif; ?>
                            </div>
                            <p class="text-gray-500 font-semibold text-sm"><?php echo e($request->location); ?></p>
                            <p class="text-xs text-gray-400">Request for <?php echo e($request->patienttype); ?> patient (Age: <?php echo e($request->patientage); ?>)</p>
                        </div>

                        <div class="sm:text-right flex-shrink-0">
                            <p class="text-sm font-bold text-gray-800 mb-1"><?php echo e($request->contactno); ?></p>
            ...
            <div class="p-6">
                ...
                <div class="space-y-3">
                    <?php $__empty_1 = true; $__currentLoopData = $topDonors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $donor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="flex items-center justify-between p-3 <?php echo e($index < 3 ? 'bg-gradient-to-r from-red-50 to-pink-50 border-l-4 border-red-400' : 'bg-gray-50'); ?> rounded-xl">
                            <div class="flex items-center gap-3">
                                <div class="flex items-center justify-center w-8 h-8 rounded-full <?php echo e($index == 0 ? 'bg-yellow-400' : ($index == 1 ? 'bg-gray-400' : ($index == 2 ? 'bg-amber-600' : 'bg-gray-200'))); ?> text-white font-bold text-sm">
                                    <?php echo e($index + 1); ?>

                                </div>
                                <div>
                                    <p class="font-semibold text-gray-900"><?php echo e($donor->name); ?></p>
                                    <p class="text-xs text-gray-500"><?php echo e($donor->email); ?></p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="font-bold text-red-600"><?php echo e($donor->total_bags ?? 0); ?> bags</p>
                            </div>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
<?php /**PATH C:\xampp\htdocs\bloodConnect\resources\views/welcome.blade.php ENDPATH**/ ?>