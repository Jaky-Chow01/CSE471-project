...
<body class="min-h-screen pb-20 relative">
    ...
    <script>
        // Real-time urgent request monitoring for admin track page
        let lastUrgentCount = 0;
        let lastUrgentTime = null;
        
        async function monitorUrgentRequests() {
            try {
                const response = await fetch('/api/dashboard/urgent-status');
                const data = await response.json();
                
                // If new urgent request detected, refresh the page
                if (data.count > lastUrgentCount || (data.last_urgent_time && data.last_urgent_time !== lastUrgentTime)) {
                    lastUrgentTime = data.last_urgent_time;
                    lastUrgentCount = data.count;
                    
                    // Show notification
                    showUrgentAlert(data.urgent_requests[0]);
                    
                    // Refresh page after short delay
                    setTimeout(() => location.reload(), 1000);
                }
            } catch (error) {
                console.error('Error monitoring urgent requests:', error);
            }
        }
        
        function showUrgentAlert(request) {
            // Create notification element
            const alert = document.createElement('div');
            alert.className = 'fixed top-4 right-4 bg-red-600 text-white px-6 py-4 rounded-2xl shadow-2xl z-50 animate-bounce max-w-xs';
            alert.innerHTML = `
                <p class="font-black text-sm mb-1">🚨 NEW URGENT REQUEST!</p>
                <p class="text-xs opacity-90">${request.bloodgroup} blood needed in ${request.location}</p>
                <p class="text-xs opacity-75 mt-1">${request.noofbags} bag(s) • Refreshing...</p>
            `;
            document.body.appendChild(alert);
        }
        
        // Check for urgent requests every 5 seconds instead of auto-refresh
        setInterval(monitorUrgentRequests, 5000);
        
        // Initial check
        monitorUrgentRequests();
    </script>
</body>
</html>
<?php /**PATH C:\xampp\htdocs\bloodConnect\resources\views/admin/track.blade.php ENDPATH**/ ?>